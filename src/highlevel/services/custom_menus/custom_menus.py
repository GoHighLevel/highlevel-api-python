...
class CustomMenus:
    ...
    async def get_custom_menu_by_id(
        self,
        custom_menu_id: str,
        ...
        """
        param_defs = [{"name": "customMenuId", "in": "path"}]
        extracted = extract_params({ "custom_menu_id": custom_menu_id }, param_defs)
        requirements = ["Agency-Access"]
        
        config: RequestConfig = {
            "method": "GET",
            "url": build_url("/custom-menus/{customMenuId}", extracted["path"]),
    ...
    async def update_custom_menu(
        self,
        custom_menu_id: str,
        request_body: UpdateCustomMenuDTO,
        options: Optional[Dict[str, Any]] = None
    ...
    async def get_custom_menus(
        ...
        """
        param_defs = [{"name": "locationId", "in": "query"}, {"name": "skip", "in": "query"}, {"name": "limit", "in": "query"}, {"name": "query", "in": "query"}, {"name": "showOnCompany", "in": "query"}]
        extracted = extract_params({ "location_id": location_id, "skip": skip, "limit": limit, "query": query, "show_on_company": show_on_company }, param_defs)
        requirements = ["Agency-Access"]
        
        config: RequestConfig = {
            "method": "GET",
            "url": build_url("/custom-menus/", extracted["path"]),
            "params": extracted["query"],
            "headers": {**extracted["header"], **(options.get("headers", {}) if options else {})},
    ...
    async def create_custom_menu(
        self,
        request_body: CreateCustomMenuDTO,
        options: Optional[Dict[str, Any]] = None
    ) -> GetSingleCustomMenusSuccessfulResponseDTO:
        """
