...
class Campaigns:
    ...
    async def get_campaigns(
        ...
        Get Campaigns
        """
        param_defs = [{"name": "locationId", "in": "query"}, {"name": "status", "in": "query"}]
        extracted = extract_params({ "location_id": location_id, "status": status }, param_defs)
        requirements = ["bearer"]
        
        config: RequestConfig = {
            "method": "GET",
            "url": build_url("/campaigns/", extracted["path"]),
